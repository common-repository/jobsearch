<?php /** Smarty Internal Plugin @package    Smarty @subpackage Cacher */

/** Cache Handler API @package    Smarty @subpackage Cacher @author     Rodney Rehm */
abstract class Smarty_CacheResource_Custom extends Smarty_CacheResource
{
    ...
    /** Save content to cache @param string $id unique cache content identifier @param string       $name       template name @param string       $cache_id   cache id @param string       $compile_id compile id @param integer|null $exp_time   seconds till expiration or null @param string       $content    content to cache @return boolean      success */
    abstract protected function save($id, $name, $cache_id, $compile_id, $exp_time, $content);

    /** Delete content from cache @param string|null $name template name @param string|null  $cache_id   cache id @param string|null  $compile_id compile id @param integer|null $exp_time   seconds till expiration time in seconds or null @return integer      number of deleted caches */
    ...
    /** populate Cached Object with timestamp and exists from Resource @param Smarty_Template_Cached $cached @return void */
    ...
    /** Read the cached template and process the header @param \Smarty_Internal_Template $_smarty_tpl do not change variable name, is used by compiled template @param Smarty_Template_Cached    $cached      cached object @param boolean                   $update      flag if called because cache update @return boolean                 true or false if the cached content does not exist */
    ...
    /** Read cached template from cache @param Smarty_Internal_Template $_template template object @return string|boolean  content */
    ...
    /** Check is cache is locked for this template @param Smarty $smarty Smarty object @param Smarty_Template_Cached $cached cached object @return boolean               true or false if cache is locked */
